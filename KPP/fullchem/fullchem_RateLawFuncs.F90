...
CONTAINS
  ...
  FUNCTION SO2uptkbyTMIO2( H ) RESULT (k)
    ...
    srMw = SR_MW(ind_SO2)
    ! Uptake by tropospheric sulfate and organics
    IF ( RELHUM >= CRITRH2  ) THEN
          gamma = H%gamma_SO2_TMI

          k_ex1 = k_ex1 + Ars_L1k( H%xArea(SUL), H%xRadi(SUL), gamma, srMw )
          k     = k_ex1 + Ars_L1k( H%xArea(ORC), H%xRadi(ORC), gamma, srMw )
    ENDIF
  ...
  FUNCTION SO2uptkbyH2O2( H) RESULT ( k )
    !
    ! Computes the hydrolysis reaction rate [1/s] of SO2 rx with H2O2.
    !
    TYPE(HetState), INTENT(IN) :: H                 ! HetChem State
    REAL(dp)                   :: srMw, k, k_ex1    ! sqrt(mol wt), rxn rate [1/s]
    REAL(dp)                   :: gamma
    ...
    IF ( RELHUM .ge. CRITRH2 ) THEN       
       ...
       k_ex1 = k_ex1 + Ars_L1k( H%xArea(SUL), H%xRadi(SUL), gamma, srMw )
       k_ex1 = k_ex1 + Ars_L1k( H%xArea(ORC), H%xRadi(ORC), gamma, srMw )
  ...
  FUNCTION SO2uptkbyNO2( H) RESULT ( k )
    !
    ! Computes the hydrolysis reaction rate [1/s] of SO2 rx with NO2.
    !
    TYPE(HetState), INTENT(IN) :: H                 ! HetChem State
    REAL(dp)                   :: srMw, k, k_ex1    ! sqrt(mol wt), rxn rate [1/s]
    REAL(dp)                   :: gamma
  ...
  FUNCTION SO2uptkbyCH2O( H) RESULT ( k )
    !
    ! Computes the hydrolysis reaction rate [1/s] of SO2 rx with CH2O.
    !
    TYPE(HetState), INTENT(IN) :: H                 ! HetChem State
    REAL(dp)                   :: srMw, k, k_ex1    ! sqrt(mol wt), rxn rate [1/s]
    REAL(dp)                   :: gamma
